...
$(".move-selector").change(function () {
	...
	syncTopCritToggle(moveGroupObj.children(".move-crit")[0]);
...
function matchesSetSearch(option, term) {
	if (!term) {
		return true;
	}
	var terms = term.toUpperCase().split(/\s+/).filter(Boolean);
	var searchFields = [
		option.pokemon,
		option.set,
		option.nickname
	...
	return terms.every(function (searchTerm) {
		for (var i = 0; i < searchFields.length; i++) {
			...
			if (field.indexOf(searchTerm) === 0 || field.indexOf("-" + searchTerm) >= 0 || field.indexOf(" " + searchTerm) >= 0) {
				return true;
			}
		}
		return false;
...
function loadDefaultLists() {
	$(".set-selector").select2({
		...
		query: function (query) {
			...
			var hasSearchTerm = query.term && query.term.trim();
			for (var i = 0; i < options.length; i++) {
				var option = options[i];
				if (!matchesSetSearch(option, query.term)) {
					continue;
				}
				if ($("#randoms").prop("checked")) {
					if (option.id) results.push(option);
				} else if (!hasSearchTerm || option.id) {
					results.push(option);
				}
			}
			query.callback({
...
function loadCustomList(id) {
	$("#" + id + " .set-selector").select2({
		...
		query: function (query) {
			...
			for (var i = 0; i < options.length; i++) {
				var option = options[i];
				if (option.isCustom && option.set && matchesSetSearch(option, query.term)) {
					results.push(option);
				}
			}
			query.callback({
...
function syncTopCritToggle(critInput) {
	if (!critInput || !critInput.dataset.topCritId) {
		return;
	}
	var topCritInput = document.getElementById(critInput.dataset.topCritId);
	if (topCritInput) {
		topCritInput.checked = critInput.checked;
	}
}

function setupTopCritToggles() {
	var sides = [
		{ resultPrefix: "L", critPrefix: "L" },
		{ resultPrefix: "R", critPrefix: "R" }
	];
	for (var s = 0; s < sides.length; s++) {
		for (var i = 1; i <= 4; i++) {
			var row = document.getElementById("resultMove" + sides[s].resultPrefix + i);
			var critInput = document.getElementById("crit" + sides[s].critPrefix + i);
			if (!row || !critInput) {
				continue;
			}
			row = row.parentNode;
			if (row.querySelector(".top-result-crit")) {
				continue;
			}
			var damageSpan = row.querySelector("span");
			if (!damageSpan) {
				continue;
			}
			var topCritInput = document.createElement("input");
			topCritInput.type = "checkbox";
			topCritInput.className = "visually-hidden";
			topCritInput.id = "critTop" + sides[s].critPrefix + i;

			var topCritLabel = document.createElement("label");
			topCritLabel.className = "btn crit-btn top-result-crit";
			topCritLabel.setAttribute("for", topCritInput.id);
			topCritLabel.textContent = "Crit";
			topCritLabel.title = "Force this attack to be a critical hit?";

			topCritInput.addEventListener("change", (function (input, linkedCritInput) {
				return function () {
					linkedCritInput.checked = input.checked;
					$(linkedCritInput).change();
				};
			})(topCritInput, critInput));

			critInput.dataset.topCritId = topCritInput.id;
			critInput.addEventListener("change", (function (linkedCritInput) {
				return function () {
					syncTopCritToggle(linkedCritInput);
				};
			})(critInput));
...
function changeBoostBy(boostSelect, delta) {
	var current = parseInt(boostSelect.value, 10);
	if (isNaN(current)) {
		current = 0;
	}
	var next = Math.max(-6, Math.min(6, current + delta));
	if (next === current) {
		return;
	}
	boostSelect.value = String(next);
	$(boostSelect).change();
}

function setupStatBoostButtons() {
	var boostSelects = document.querySelectorAll(".poke-info .boost");
	for (var i = 0; i < boostSelects.length; i++) {
		var boostSelect = boostSelects[i];
		if (boostSelect.parentNode.querySelector(".boost-stepper")) {
			continue;
		}
		var stepper = document.createElement("span");
		stepper.className = "boost-stepper";

		var upBtn = document.createElement("button");
		upBtn.type = "button";
		upBtn.className = "btn boost-step-btn";
		upBtn.textContent = "+";
		upBtn.title = "Raise stat stage";
		upBtn.addEventListener("click", (function (select) {
			return function () {
				changeBoostBy(select, 1);
			};
		})(boostSelect));

		var downBtn = document.createElement("button");
		downBtn.type = "button";
		downBtn.className = "btn boost-step-btn";
		downBtn.textContent = "-";
		downBtn.title = "Lower stat stage";
		downBtn.addEventListener("click", (function (select) {
			return function () {
				changeBoostBy(select, -1);
			};
		})(boostSelect));
		...
		boostSelect.parentNode.appendChild(stepper);
	}
}

function applyTeamBoxSearchFilter() {
	var searchInput = document.getElementById("team-box-search");
	if (!searchInput) {
		return;
	}
	var rawTerms = searchInput.value.toUpperCase().trim().split(/\s+/);
	var searchTerms = [];
	for (var i = 0; i < rawTerms.length; i++) {
		if (rawTerms[i]) {
			searchTerms.push(rawTerms[i]);
		}
	}
	var mons = document.querySelectorAll(".trainer-pok.left-side");
	for (var j = 0; j < mons.length; j++) {
		var mon = mons[j];
		var monText = (mon.getAttribute("data-id") || "").toUpperCase();
		var isMatch = searchTerms.length === 0 || searchTerms.every(function (term) {
			return monText.indexOf(term) !== -1;
		});
		mon.hidden = !isMatch;
	}
}
...
function maybeAutoRefreshColorCode() {
	var autoRefresh = document.getElementById("cc-auto-refr");
...
$(document).ready(function () {
	...
	$('#team-box-search').on('input keyup', applyTeamBoxSearchFilter);
	applyTeamBoxSearchFilter();
